...
- **Credit contributors with a thank-you comment IN OUR OWN REPO**: when using someone's work (repo, pattern, blog post, library, gist) in a proposal / PR / issue / docs change, IMMEDIATELY post a comment **on OUR PR/issue** (the one in `ievo-ai/*`, after opening it) that (1) @-mentions the author or org by their actual GitHub handle, (2) links to the specific upstream work that inspired the change, (3) thanks them concretely (one line describing what their work enabled here). Format: `🙏 Credit: this <PR/issue> was inspired by @<handle>'s [<project>](<url>) — <what it enabled>. Thanks for shipping publicly.` **Do NOT post the credit in the contributor's own repo** — unsolicited comments on someone else's surface read as spam even when well-intentioned, especially from automation. GitHub's @-mention mechanism handles notification: the @-mention in OUR PR sends them a real notification, they can navigate to our thread and see the credit in context with the actual code/PR they can inspect. Skip credit only when the source is an official Anthropic / OpenAI / Google announcement of their own product (no individual contributor to thank). Community work, library authors, blog posts, independent open-source repos — always credit. A body-text mention is NOT enough; the credit must be a separate public comment so the author actually sees the notification (body edits don't trigger @-mention pings reliably). Observed 2026-05-22: (a) Eva proposed skills#53 citing `DenisSergeevitch/agents-best-practices` in the body but did not post a thank-you comment — operator caught it as a violation. (b) On the follow-up, an iEvo agent tried to post the thank-you as a new issue **in DenisSergeevitch's own repo** — operator corrected: «не надо постить в чужие репы, только в свои, с тагом юзернейма». Both halves of this rule (post comment, post it in OUR repo) need to land together.
