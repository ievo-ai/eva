...
```yaml
last_scan: 2026-06-20T07:37:47Z
status: changed
run_id: 27864303770
```

**Summary:** Claude Fable 5 and Mythos 5 launched June 9, then suspended by US government export control directive on June 12, 2026. Models currently unavailable. The `fable` vendor-neutral alias (PR #215 open in ievo-ai/skills) resolves to a currently-suspended model for users in affected jurisdictions.

History:
- 2026-06-20T07:37:47Z — changed: Fable 5 + Mythos 5 launched June 9, suspended June 12 by US export control directive; `fable` alias functional but model inaccessible in affected jurisdictions; no new Claude Code agent-format changes
...
**Summary:** v2.1.162–v2.1.183 (June 2–20). Key additions for iEvo: v2.1.183 blocks destructive git commands in auto mode + WebSearch-in-subagents fix (#219, #221); v2.1.181 `/config key=value` inline settings (#218); v2.1.178 nested `.claude/skills` loading + `Tool(param:value)` permission syntax (#209, #208); v2.1.176 hook `if`-condition for path patterns (#201); v2.1.175 `enforceAvailableModels` managed setting (#197); v2.1.172 `availableModels` enforcement + sub-agent nesting up to 5 levels (#195, #194); v2.1.170 Claude Fable 5 (now suspended); v2.1.169 `/cd`, `disableBundledSkills`, `--safe-mode` (#193, #190, #189); v2.1.162 effort: persisted to settings / warning→error debate (#187). Most signals filed as issues in ievo-ai/skills.

History:
- 2026-06-20T07:37:47Z — changed: v2.1.162–183; destructive git blocking + WebSearch fix (#219, #221); /config (#218); nested .claude/skills (#209); Tool(param:value) (#208); hook if-condition (#201); enforceAvailableModels (#197); availableModels + sub-agent depth (#195, #194); Fable 5 (suspended June 12); /cd + disableBundledSkills + --safe-mode (#193, #190, #189)
...
**Summary:** v1.0.133 (2026-05-23) remains the latest release observed. No new releases detected since last scan. OIDC Workload Identity Federation still the last addition. Eva's own workflows (eva#65) still need auditing for v0.x deprecated inputs.

History:
- 2026-06-20T07:37:47Z — unchanged: not re-fetched; still v1.0.133 as of June 1 scan; eva#65 still open
...
History:
- 2026-06-20T07:37:47Z — unchanged: not re-fetched; redirects to code.claude.com per prior scan; tracking sub-pages directly
...
**Summary:** rust-v0.141.0 (2026-06-18) — Remote executors use Noise-protocol encrypted relay channels for cross-platform execution; executor plugins can activate stdio MCP servers per thread (new capability for per-agent MCP isolation); app-server clients can list child threads and correlate external-agent imports (relevant for handoff); tool-search caching reduces latency in tool-heavy sessions; TLS P-521 support (enterprise proxy). rust-v0.140.0 — skills decoupled from core (filed as #210); rust-v0.138.0 — codex plugin marketplace --json (filed as #196). Most signals filed as issues #196–#216.

History:
- 2026-06-20T07:37:47Z — changed: rust-v0.141.0 (encrypted Noise relay, per-thread MCP activation, child thread listing for app-server, tool-search caching, TLS P-521); triggered F-2026-06-20-001, F-2026-06-20-002
...
History:
- 2026-06-20T07:37:47Z — unchanged: no new agent tooling posts relevant to skills repo
...
History:
- 2026-06-20T07:37:47Z — unchanged: spec stable; PRs #380, #386, #345 still pending; no new fields
...
History:
- 2026-06-20T07:37:47Z — unchanged: PRs #380, #386, #345 still open; no new merges since May 20; spec stable
...
**Summary:** v3.8 (2026-06-18) — `/automate` skill for local agent sessions, Slack emoji triggers, 5 new GitHub triggers, computer use for cloud agents (filed as #220). v3.7 (2026-06-17) — `/in-cloud` command for running cloud subagents in isolated VMs, `/babysit` command for PR prep, background cloud agent execution (NOT yet filed; `/in-cloud` isolation is relevant to /ievo:security-check sandboxing — triggered F-2026-06-20-002). v3.7 SDK update (2026-06-04) — custom tools via function definitions, auto-review routing with configurable permissions.json (filed as #213). v3.6 (2026-05-29) — Auto-review Run Mode with classifier subagent (triggered #164).

History:
- 2026-06-20T07:37:47Z — changed: v3.8 (June 18 /automate → #220); v3.7 (June 17 /in-cloud isolated VM subagents → F-2026-06-20-002); v3.7 SDK (June 4 configurable permissions → #213); v3.6 classifier still open #164
...
History:
- 2026-06-20T07:37:47Z — unchanged: no relevant AI agent tooling posts on front page at scan time
...
**Summary:** Last commit 2026-05-30 (workflow orchestration guidance, +261 lines in references/workflow-orchestration.md). No new commits since June 1 scan. Relevant to skills#162 (/ievo:workflow skill). 15 files in references/ confirmed.

History:
- 2026-06-20T07:37:47Z — unchanged: no new commits since May 30; references/workflow-orchestration.md still the latest addition; skills#162 still open
...
**Summary:** Spec stable as of last scan. Key fields: `name`, `description`, `effort:`, `disallowed-tools`, `allowed-tools`, `hooks`, dynamic `!`command`` context injection, `$CLAUDE_SKILL_DIR/$CLAUDE_SESSION_ID` vars. Skills in `.claude/skills/` auto-loaded (v2.1.157+). Body ≤500 lines. Not re-fetched this run — tracking CC releases via github.com/anthropics/claude-code/releases.

History:
- 2026-06-20T07:37:47Z — unchanged: not re-fetched; spec stable from June 1 scan; no new fields indicated by CC v2.1.162-183 release notes
...
History:
- 2026-06-20T07:37:47Z — unchanged: not re-fetched; spec stable from June 1 scan
...
History:
- 2026-06-20T07:37:47Z — unchanged: not re-fetched; still research preview from June 1 scan; no GA announcement observed in CC release notes
...
**Summary:** Sub-agents documentation for Claude Code. Model resolution order: (1) `CLAUDE_CODE_SUBAGENT_MODEL` env var if set, (2) per-invocation parameter, (3) agent frontmatter `model:`, (4) main-conversation model. Dispatch via Task tool. `agent:` field in `settings.json` (v2.1.157) adds a fourth override path. Note: v2.1.172+ adds 5th path via `availableModels` enforcement (filed as #195); v2.1.175 adds `enforceAvailableModels` managed setting (filed as #197). Sub-agent nesting up to 5 levels added in v2.1.172 (filed as #194).

History:
- 2026-06-20T07:37:47Z — unchanged: not re-fetched; spec stable from June 1 scan; new bypass vectors via availableModels (#195) and enforceAvailableModels (#197) filed separately
