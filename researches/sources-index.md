...
```yaml
last_scan: 2026-06-04T07:54:09Z
status: unchanged
run_id: 26938373272
```

**Summary:** Jun 3 announcements: Claude Partner Network Services Track, AI-Enabled Cyber Threats Research (with MITRE), Expanding Project Glasswing. None are agent-platform API changes. Tracking Claude Code changes via release notes directly.

History:
- 2026-06-04T07:54:09Z — unchanged: Jun 3 announcements (Partner Network, Glasswing expansion, cyber threats research) — no model releases or CC API changes
...
**Summary:** v2.1.162 (2026-06-03) — `claude agents --json` adds `waitingFor` field; WebFetch permission rules fix; Windows path matching fix (backslash + case-variant); MCP secret redaction in `claude mcp`; quieter startup. v2.1.161 (2026-06-02) — `OTEL_RESOURCE_ATTRIBUTES` as metric labels (triggered skills#171 already open); fork subagents (`/fork`, `CLAUDE_CODE_FORK_SUBAGENT`) enabled by default; parallel tool-call independence (failed Bash no longer cancels batch). v2.1.160 (2026-06-02) — `acceptEdits` prompts for `.pre-commit-config.yaml` + other build-tool configs (triggered F-2026-06-02-002).

History:
- 2026-06-04T07:54:09Z — changed: v2.1.161 OTEL labels + fork subagents default-enabled; v2.1.162 claude agents --json waitingFor, WebFetch permission fix, Windows path matching fix, MCP secret redaction
...
**Summary:** v1.0.135 (2026-06-03, latest) — bumps Claude Code CLI to 2.1.162 and Agent SDK to 0.3.162. v1.0.134 (2026-06-02) — added workload identity federation support to base-action (OIDC via GH OIDC provider); bumped `actions/setup-node` to v6.4.0 (Node.js 24). Eva's own workflows (eva#65) still need auditing for v0.x inputs.

History:
- 2026-06-04T07:54:09Z — changed: v1.0.134 OIDC workload identity federation + Node.js 24 bump; v1.0.135 Claude Code 2.1.162 + Agent SDK 0.3.162 bump
...
**Summary:** rust-v0.137.0 (2026-06-04) — plugin workflows: `codex plugin list --json` output, cached remote catalog suggestions; plugin loading deduplicates local/remote installs, treats malformed `skills` fields as warnings; multi-agent v2 keeps runtime choice per thread; shared prompts/skills plumbing moved into dedicated crates. rust-v0.136.0 (2026-06-01) — hook output event schema tightened (breaking; triggered F-2026-06-02-001); runtime extra skill roots API; multi-agent assignment tool renamed; /archive slash command; memories moved to SQLite.

History:
- 2026-06-04T07:54:09Z — changed: rust-v0.137.0 (2026-06-04) plugin list --json, dedup, malformed skills warnings, multi-agent v2; rust-v0.138.0-alpha.1 (pre-release, no notes)
...
History:
- 2026-06-04T07:54:09Z — unchanged: not re-fetched; no indication of agent tooling changes since last scan
...
History:
- 2026-06-04T07:54:09Z — unchanged: spec stable; PRs #380, #386, #345 still open and unmerged; no new fields
...
History:
- 2026-06-04T07:54:09Z — unchanged: last commit still 2026-05-20; PRs #380, #386, #345 still open and unmerged
...
**Summary:** Last versioned release: v3.6 (2026-05-29) — Auto-review Run Mode with classifier subagent (triggered #164). Jun 3 announcement: enterprise-only Organizations hierarchy feature (not agent-format relevant). No new versioned release since v3.6.

History:
- 2026-06-04T07:54:09Z — unchanged: no new product release since v3.6 (May 29); Jun 3 was enterprise Organizations feature (not relevant)
...
**Summary:** Front page at fetch time had no posts matching `claude code`, `codex`, `agent skills`, or `MCP`. Notable: "The ways we contain Claude across products" (Anthropic safety, rank 8) and "Launch HN: Hyper (YC P26) – Company brain for agentic development" (rank 24). HN is point-in-time; limited signal. Consider fetching HN Algolia search API instead for topic-filtered results.

History:
- 2026-06-04T07:54:09Z — unchanged: no posts specifically on claude code/codex/MCP; agentic dev (Hyper YC P26) and Claude containment on front page
...
**Summary:** Last commit 2026-05-30 — "Add workflow orchestration guidance" (+261 lines, relevant to open #162). No new commits since 2026-06-02. v1.2.0 with Claude Code skill support documentation.

History:
- 2026-06-04T07:54:09Z — unchanged: last commit still 2026-05-30; no new commits since last scan
...
**Summary:** Major new frontmatter fields (post 2026-06-02): `paths` — glob patterns limiting auto-activation to matching file context; `when_to_use` — additional trigger hints (combined with description, counts toward 1,536-char cap); `shell` — bash or powershell for `!command` blocks; `arguments` frontmatter with named `$name` / `$ARGUMENTS[N]` substitution. New settings: `skillOverrides` (4 states: on/name-only/user-invocable-only/off), `disableSkillShellExecution` policy. New substitution vars: `${CLAUDE_EFFORT}`, `${CLAUDE_SKILL_DIR}`. Three new bundled skills: /run, /verify, /run-skill-generator (require v2.1.145+). Prior: `disallowed-tools`, `effort:`, `.claude/skills/` auto-load.

History:
- 2026-06-04T07:54:09Z — changed: new frontmatter fields paths/when_to_use/shell/arguments; skillOverrides 4-state setting; disableSkillShellExecution policy; new bundled skills /run /verify /run-skill-generator; triggered F-2026-06-04-001, F-2026-06-04-002, F-2026-06-04-003
...
**Summary:** Still "research preview". New since last scan: one-off runs (single-time trigger, auto-disables, doesn't count against daily cap); `/schedule list`, `/schedule update`, `/schedule run` now documented; environment setup script caching; GitHub trigger `matches regex` operator (`.*hotfix.*` not just `hotfix`); usage credits for metered overage; API trigger beta header `anthropic-beta: experimental-cc-routine-2026-04-01`; network access detail expanded (MCP traffic via Anthropic servers).

History:
- 2026-06-04T07:54:09Z — changed: one-off runs, /schedule list/update/run, env caching, regex trigger, API beta header documented
...
**Summary:** Still "research preview" with rolling gradual rollout. New since last scan: iMessage plugin added (third channel alongside Telegram/Discord; macOS only, reads ~/Library/Messages/chat.db, requires Full Disk Access); `allowedChannelPlugins` managed setting for enterprise custom allowlists; fakechat localhost demo quickstart; Permission relay capability (channel can forward permission prompts to remote users). Enterprise controls table documented: `channelsEnabled` + `allowedChannelPlugins`. `--dangerously-load-development-channels` flag for custom channel testing.

History:
- 2026-06-04T07:54:09Z — changed: iMessage plugin added; allowedChannelPlugins enterprise setting; fakechat demo; permission relay capability
...
**Summary:** New fields since last scan: `isolation: worktree` (isolated git worktree, auto-cleaned); `color` (display color: red/blue/green/yellow/purple/orange/pink/cyan); `initialPrompt` (auto-submitted first turn when agent runs as main session); `memory` (persistent scope: user/project/local, enables MEMORY.md). New capabilities: fork subagents (`/fork`, inherit full conversation history, enabled by default v2.1.161); `--agents` CLI flag (ephemeral JSON agent defs); `Agent(agent_type)` tool syntax; plugin subfolder scoping (`agents/review/security.md` → `my-plugin:review:security`). Persistent transcripts at `~/.claude/projects/{project}/{sessionId}/subagents/agent-{agentId}.jsonl`. Prior: `effort:`, `disallowedTools`, `permissionMode`, `maxTurns`, `skills`, `mcpServers`, `hooks`, `background`.

History:
- 2026-06-04T07:54:09Z — changed: isolation:worktree, color, initialPrompt, memory new fields; fork subagents enabled by default; --agents CLI; plugin subfolder scoping; persistent transcripts
...
**Summary:** Same content as code.claude.com/docs/en/sub-agents (same page). New since last scan: same fields as sub-agents section above (isolation:worktree, color, initialPrompt, memory, fork subagents, --agents CLI). Key security note still relevant: `CLAUDE_CODE_SUBAGENT_MODEL` env var and `agent:` settings.json field both bypass `security-auditor.md` model frontmatter. New bypass via `skillOverrides` setting documented in skills.md.

History:
- 2026-06-04T07:54:09Z — changed: same new fields as sub-agents; fork subagents enabled default v2.1.161; skillOverrides new bypass path noted
