...
## F-2026-06-22-001 — Codex v0.141.0 MCP tool timeout 300s — document minimum version for parallel security-auditor

```yaml
id: F-2026-06-22-001
discovered_at: 2026-06-22T08:22:56Z
run_id: 27939067190
target_repo: ievo-ai/skills
title: Document Codex v0.141.0 MCP tool timeout increase (60s→300s) as minimum version requirement for parallel security-auditor reliability
status: issued
issue_url: https://github.com/ievo-ai/skills/issues/227
effort: low
scope: multi-file
evidence:
  - https://github.com/openai/codex/releases: rust-v0.141.0 (2026-06-18) increased default MCP tool timeout from 60s to 300s — iEvo's parallel security-auditor dispatches perform deep multi-file reads that can exceed 60s on large repos, causing silent sub-agent termination without a security verdict
```

Codex rust-v0.141.0 (2026-06-18) increased the default MCP tool timeout from 60 seconds to 300 seconds. iEvo's `/ievo:init` dispatches parallel `security-auditor` sub-agents via the Task tool — each performs a deep, multi-file read of a candidate skill repo. Deep security scans of large repos can exceed 60 seconds. On Codex < v0.141.0, these sub-agents silently time out, producing incomplete or missing verdicts without surfacing an error to the user. Users may proceed to install a candidate that was never fully audited.

**AGENTS.md § Security model** should add a bullet after the `CLAUDE_CODE_SUBAGENT_MODEL` note:
> **Codex MCP tool timeout — operator gotcha.** Prior to Codex rust-v0.141.0 (2026-06-18), MCP tool calls timed out after 60 seconds. `security-auditor` performs deep multi-file reads that can exceed this window on large repos, causing silent sub-agent termination without a verdict. Codex ≥ v0.141.0 is required for reliable parallel security-auditor dispatch. Upgrade via `codex update`.

**`security-check/SKILL.md` compatibility field** should append: `Codex: requires rust-v0.141.0+ — prior versions had a 60s MCP tool timeout insufficient for deep repo scans.`

Files: `AGENTS.md` + `plugins/ievo/skills/security-check/SKILL.md`. No scripts. Version bump + CHANGELOG.md entry required.

---

## F-2026-06-22-002 — CC v2.1.183 attribution.sessionUrl — prevent session-link leakage in automation PRs
...
run_id: 27939067190
target_repo: ievo-ai/skills
title: Document CC v2.1.183 attribution.sessionUrl setting in AGENTS.md § Issue lifecycle to prevent session-link leakage in automation-generated public PRs
status: issued
issue_url: https://github.com/ievo-ai/skills/issues/228
effort: low
scope: single-file
evidence:
  - https://github.com/anthropics/claude-code/releases: v2.1.183 — attribution.sessionUrl setting added; when set to false omits the claude.ai session link from commits and PRs generated by Claude Code automation; ievo-ai/skills is a public repo where such links appear in public PR history
```

Claude Code v2.1.183 introduced `attribution.sessionUrl` — setting it to `false` in `.claude/settings.json` omits the `claude.ai/code/runs/<session-id>` link from automation-generated PR bodies. iEvo's `issue-handler.yml` and `conflict-resolver.yml` create public PRs on `ievo-ai/skills`. By default these PRs include a session URL exposing internal automation session identifiers in the public git history. AGENTS.md § "Public-repo content safety" explicitly prohibits "internal endpoints, hostnames, dashboard URLs, internal-only paths" in public artefacts — a session URL is borderline-applicable.

**AGENTS.md § Issue lifecycle** should add one sentence after "Never auto-merges — human must review and merge":
> **Session link suppression (CC v2.1.183+).** Add `"attribution": { "sessionUrl": false }` to `.claude/settings.json` to suppress the `claude.ai/code/runs/<session-id>` link from handler-generated PR bodies — these session identifiers appear in the public repo PR history and are covered by the public-repo content safety rule.

Single-file, documentation-only. Version bump + CHANGELOG.md entry required.

---

## F-2026-06-22-003 — Cursor v3.8 computer use default in cloud agents — security-check/SKILL.md gap
...
run_id: 27939067190
target_repo: ievo-ai/skills
title: Add Cursor v3.8 cloud agent computer use restriction guidance to security-check/SKILL.md
status: issued
issue_url: https://github.com/ievo-ai/skills/issues/229
effort: low
scope: single-file
evidence:
  - https://www.cursor.com/changelog: v3.8 (2026-06-18) — computer use tool enabled by default for cloud agents (/in-cloud); security-check runs deep review of untrusted third-party code — computer use is an exfiltration surface not covered by disallowed-tools (which is Claude Code-only)
  - /tmp/skills/plugins/ievo/skills/security-check/SKILL.md: disallowed-tools frontmatter blocks Write/Edit/Bash(rm*) but not computer use; disallowed-tools is Claude Code-specific and has no effect on Cursor hosts
```

Cursor v3.8 enabled the computer use tool by default for cloud agents (deployed via `/in-cloud` from v3.7). `security-check/SKILL.md` has `disallowed-tools` frontmatter for read-only enforcement — but `disallowed-tools` is a Claude Code convention with no effect on Cursor. In a Cursor cloud agent session, a malicious skill under review could potentially use the computer use tool to interact with the desktop or capture screen state. The skill body should add a Cursor cloud agent section noting: (a) the computer use risk, (b) that `disallowed-tools` does not apply on Cursor, (c) the mitigation — restrict computer use via `permissions.json` (see open `ievo-ai/skills#213`) before running the skill in Cursor cloud mode. Single-file documentation addition. Version bump + CHANGELOG.md entry required.
